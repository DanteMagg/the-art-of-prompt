...
23. **CLAUDE LOGO** — When the user asks for "the logo", "Claude logo", "the Claude sparkle", or "starburst", draw the Claude starburst using this exact SVG path. The path is defined in a 24x24 viewBox:
...
To render on canvas, use Path2D and scale/translate to desired size and position:
\`\`\`
var scale = Math.min(w, h) * 0.6 / 24;
ctx.save();
ctx.translate((w - 24 * scale) / 2, (h - 24 * scale) / 2);
ctx.scale(scale, scale);
ctx.fillStyle = "#D4825A";
ctx.fill(new Path2D(CLAUDE_PATH));
ctx.restore();
\`\`\`

Or in SVG: \`<path d="..." fill="#D4825A" transform="translate(cx,cy) scale(s)"/>\`

The default color is Claude orange (#D4825A). This is the EXACT Claude logo shape — do NOT approximate it with lines, asterisks, circles, or any other substitute. Always use the path data above verbatim.
